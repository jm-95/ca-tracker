...
function fyList() {
  const list = [];
  for (let y = 2024; y <= 2026; y++) list.push(`FY ${y}-${String(y+1).slice(2)}`);
  return list;
}
function currentFY() {
...
function emptyStages() { return Object.fromEntries(STAGES.map(s=>[s.key,"Pending"])); }
function stageProgress(stages) {
  const vals=Object.values(stages||{}); const done=vals.filter(v=>v==="Done").length;
  const na=vals.filter(v=>v==="N/A").length; const total=vals.length-na;
...
function ensurePeriods(client, fy) {
  const periods=periodsForClient(client); const fyData=client.periods?.[fy]||{};
  const filled={...fyData};
  periods.forEach(p=>{ if(!filled[p.key]) filled[p.key]=emptyStages(); });
...
export default function Tracker({ session }) {
  ...
  const handleStageChange = async (clientId, periodKey, stageKey, value) => {
    ...
    const updated = {
      ...
      periods: {
        ...client.periods,
        [activeFY]: {
          ...client.periods?.[activeFY],
          [periodKey]: { ...client.periods?.[activeFY]?.[periodKey], [stageKey]: value }
        }
      }
    };
    // optimistic update
  ...
  const filtered = clients.filter(c => {
    ...
    const allV = Object.values(fyData).flatMap(p=>Object.values(p));
        ...
        <div style={{ display:"flex",alignItems:"center",gap:6 }}>
          {fyList().map(fy=>(
            <button key={fy} className={`fy-btn${activeFY===fy?" act":""}`}
              onClick={()=>{ setActiveFY(fy); setView("list"); setSelected(null); }}>{fy}</button>
          ))}
          <div style={{ width:1,height:18,background:"#1E293B",margin:"0 4px" }}/>
          <span style={{ fontSize:11,color:"#334155" }}>{session.user.email}</span>
          <button className="btn-g" style={{ padding:"6px 12px",fontSize:12 }} onClick={handleSignOut}>Sign out</button>
...
function DetailView({ client, activeFY, activePeriod, setActivePeriod, onEdit, onDelete, onStageChange }) {
  const [confirmDel, setConfirmDel] = useState(false);
  const periods   = periodsForClient(client);
  const fyData    = client.periods?.[activeFY]||{};
  const curStages = fyData[activePeriod]||emptyStages();
  const pct = stageProgress(curStages);
  const st  = overallStatus(curStages);
        ...
        <div className="card" style={{ padding:16 }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:`repeat(${Math.min(periods.length,6)},1fr)`,gap:4 }}>
            {periods.map(p=>{
              ...
              return (
                <div key={p.key} className="pgrid-cell"
      ...
      <div className="card" style={{ padding:18 }}>
        <div style={{ display:"flex",gap:5,overflowX:"auto",paddingBottom:12,marginBottom:14,borderBottom:"1px solid #1E293B" }}>
          {periods.map(p=>{
            ...
            const isAct=activePeriod===p.key;
            return (
              <button key={p.key} className={`ptab${isAct?" act":""}`}
                onClick={()=>setActivePeriod(p.key)}
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
          <div>
            ...
            <div style={{ fontSize:11,color:"#475569" }}>{pct}% complete · {Object.values(curStages).filter(v=>v==="Done").length}/{STAGES.length} stages done</div>
          </div>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
          {STAGES.map(stage=>{
            const val=curStages[stage.key]||"Pending";
            const ss=STATUS_STYLES[val];
            return (
              <div key={stage.key} style={{ display:"flex",alignItems:"center",gap:11,padding:"10px 13px",background:"#0A0F1E",borderRadius:9,border:`1px solid ${val!=="Pending"?ss.border+"55":"#1E293B"}` }}>
                <span style={{ fontSize:17,width:22,textAlign:"center" }}>{stage.icon}</span>
                <span style={{ flex:1,fontSize:13,fontWeight:500,color:"#CBD5E1" }}>{stage.label}</span>
                <div style={{ display:"flex",gap:4 }}>
                  {STATUS_OPTIONS.map(opt=>(
                    <button key={opt} className="spill"
                      onClick={()=>onStageChange(client.id,activePeriod,stage.key,opt)}
